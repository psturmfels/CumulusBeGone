...
for i = 1:6
    for j = 1:4
        [im, pixelLabels, pixelClusterDistances, Am] = segmentImage(testImages(j), k(i), l_weights(i), a_weight, b_weights(i), seRadius(i));
        imageArray = cat(4, imageArray, im);
        pixelLabelsArray = cat(3, pixelLabelsArray, pixelLabels);
    end
    displaySegmentations(imageArray, pixelLabelsArray, 2, 2, testNames(i));
end
...
function  [im, pixelLabels, pixelClusterDistances, Am] = segmentImage(imageName, k, l_weight, a_weight, b_weight, seRadius, imageDir)
    ...
    [im, pixelLabels, pixelClusterDistances, Am] = slic(im, k, l_weight, a_weight, b_weight, seRadius);
end
